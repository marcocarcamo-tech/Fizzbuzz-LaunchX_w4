...
app.get("/v1/fizzbuzz/:number", (request, response) => {
    ...
    const fizzbuzzValue = ExplorerController.getFizzbuzzValidationValueByNumber(number);
    const fizzbuzzValueObject = {};
    let scoreKey= "score";
    let trickKey = "trick";
    fizzbuzzValueObject[scoreKey] = number;
    fizzbuzzValueObject[trickKey] = fizzbuzzValue;
    response.json(fizzbuzzValueObject);
});
app.listen(port, () => {
